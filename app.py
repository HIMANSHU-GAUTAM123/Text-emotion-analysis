...
csv_file_path = 'models/list.csv'

# Specify the column name containing sentences
column_name = 'text'

# Create an empty list to store the sentences
sentences = []

# Open the CSV file and read the sentences from the specified column
with open(csv_file_path, 'r', newline='', encoding='utf-8') as file:
    reader = csv.DictReader(file)
    for row in reader:
        # Extract the sentence from the specified column
        sentence = row[column_name]
        sentences.append(sentence)

# Now, 'sentences' contains the list of sentences from the specified column in the CSV file
#print(sentences)
# Load Model
# pipe_lr = joblib.load(open("./models/emotion_classifier_pipe_lr.pkl", "rb"))
pipe_lr =load_model('models/CNN_biLSTM_dataset2final.h5')
...
def predict_emotions(docx):
    ...
    seq = tokenizer.texts_to_sequences([docx])
    print(seq)
    print([docx])
    padded = pad_sequences(seq, maxlen=max_seq_len)
    results = pipe_lr.predict(padded)
    print(results)
    x=np.argmax(results)
    print(x)
    if(x==0):
...
emotions_emoji_dict = {"joy": "😂", "fear": "😨😱","anger": "😠",  "sadness": "😔","neutral": "😐","surprise": "😮","love": "🤗"}
    ...
    if choice == "Home":
        ...
        if submit_text:
            ...
            with col2:
                ...
                proba_df = pd.DataFrame(probability, columns=class_names)
